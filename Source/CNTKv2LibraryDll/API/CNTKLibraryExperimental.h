...
namespace CNTK {
    ...
    {
        SequenceDataBase(unsigned int numberOfSamples, bool isValue)
            : m_numberOfSamples(numberOfSamples), m_elementType(DataType::Unknown), m_isValid(isValue)
        {}
    ...
    {
        DenseSequenceData(unsigned int numberOfSamples = 1, bool isValue = true)
            : SequenceDataBase(numberOfSamples, isValue)
        {}
    };
    ...
    {
        SparseSequenceData(unsigned int numberOfSamples = 1, bool isValue = true)
            : SequenceDataBase(numberOfSamples, isValue)
        {}
    ...
    /// only a limited number of chunks are kept in memory. It uses only sequences from this chunks for randomization.
    ///
    ...
    typedef std::vector<ChunkInfo> ChunkInfos;
